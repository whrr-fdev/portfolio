...
(function() {
    ...
    window.addEventListener('load', () => {
        ...
        setTimeout(() => {
            f.classList.add('flash'); // Вспышка
            
            setTimeout(() => {
                l.style.display = 'none'; // Убираем лого
                f.classList.remove('flash'); // Убираем вспышку
                f.style.display = 'none'; // Полностью удаляем слой, чтобы кликалось
                document.body.classList.remove('stop-scrolling'); // Разрешаем скролл
            }, 200);
        }, 700); // Задержка появления основного сайта
    });
})();
// Инициализация темы
